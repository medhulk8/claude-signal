...
- **LLM (Groq):** pipeline-generated only. Explanations stored in digest.json. Extension is client-only, no API keys.
...
- `explanation` field in digest.json: `{ what_it_is, why_it_matters, how_to_use[] }` — generated by pipeline, read-only in extension
...
3. Renders `explanation.what_it_is`, `explanation.why_it_matters`, `explanation.how_to_use[]`
4. If no explanation yet → shows graceful fallback ("Explanation will appear after next pipeline run")
5. Marks item as read on open
...
- **Groq key:** stored in GitHub Secrets as `GROQ_API_KEY`, used only by the pipeline, never in the extension
